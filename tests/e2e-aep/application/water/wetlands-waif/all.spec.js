...
describe(harness.getCommandLineArgs().appType + ':' + harness.getCommandLineArgs().appSubType +
    ...
    OneStop.pageAuthorizationAdditionalInformationWetlandsWaif(
        false,
        'WAIF - Incidental activities',
        ...
        require('path').join(__dirname, '/attachments/WaifWetlandReclamation.pdf'),
        require('path').join(__dirname, '/attachments/WaifWetlandReplacement.pdf'),
        require('path').join(__dirname, '/attachments/WaifPermitteeResponsible.pdf'),
        [
            {
                RWVAU: '5',
                replacementArea: '1',
