...
describe(harness.getCommandLineArgs().appType + ':' + harness.getCommandLineArgs().appSubType +
    ...
    describe('open map from url', () => {
        it('open direct link', async () => {
            driver.get(configJson.gisBasePublicUrl + '/Onestop/Public/index.html');
	        await OneStop.waitForObjectLoad('.modal-container.terms-of-use', waitLong * 10, waitLong);
        });
	...
	describe('verify toolbar menu', () => {
		for (let i = 0; i < mapToolsMenu.length; i++) {
			OneStop.clickMapToolsButtons(mapToolsMenu[i].clickToolsButton,
				mapToolsMenu[i].toolsSubmenuToClick,
				mapToolsMenu[i].expectedCSS,
				mapToolsMenu[i].expectedText,
				mapToolsMenu[i].closeSideMenu);
		}
	});
